...
set number "显示行号
syntax on "语法高亮
set tabstop=2 "设置tap键为4个空格
set shiftwidth=2 "设置当行之间交错时使用4个空格， << 和 >> 命令移动时的宽度为 4
...
set relativenumber "显示相对行号，而不是绝对行号
set guioptions-=r " 隐藏右侧垂直滚动条
" set undofile "编辑一个文件的时候要创建<FILENAME>.un~文件,以便撤销，即使是关闭重开文件
...
autocmd! bufwritepost .vimrc source ~/.vimrc "定义了一个自动命令，每次写入.vimrc后，都会执行这个自动命令，source一次~/.vimrc文件
set noimd "避免中文干扰
